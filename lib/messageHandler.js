...
async function handleAntiLink(ctx) {
  ...
  const linkPattern = /(chat\.whatsapp\.com|https?:\/\/[^\s]+)/i;
  if (!linkPattern.test(ctx.text)) return false;
  try {
    ...
    await ctx.reply(`🔗🚫 @${ctx.sender.split('@')[0]}, links aren't allowed here.`);
    return true;
  } catch {
    return false;
  }
...
async function handleMessagesUpsert({ messages }, sock, commands) {
  for (const msg of messages) {
    ...
    if (await handleAntiLink(ctx)) continue;
